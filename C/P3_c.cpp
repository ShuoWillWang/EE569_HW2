...
void Skeleton(int BytesPerPixel, int Size1, int Size2, unsigned char *ImagedataP, unsigned char *Imagedata)
{
	Ext_i(BytesPerPixel, Size1, Size2, ImagedataP, ImagedataPex);
	int dist = 1;
	do//for (int g = 0; g < 44; g++)//
	{
		dist = 0;
			...
			for (int i = 0; i < Size2; i++)
			{
				for (int j = 0; j < Size1; j++)//check if a pixel can be the candidate of the delete
				{
					...
					else
					{
						...
						{
							if (ind == 232 || ind == 105 || ind == 240 || ind == 212 || ind == 150 || ind == 23 || ind == 15 || ind == 43)
							{
								ImagedataPexM[BytesPerPixel * ((Size1 + 2) * (i + 1) + (j + 1)) + s] = 1;
							}
							else
						...
						{
							if (ind == 251 || ind == 254 || ind == 223 || ind == 127)
							{
								ImagedataPexM[BytesPerPixel * ((Size1 + 2) * (i + 1) + (j + 1)) + s] = 1;
							}
							else
			...
			for (int i = 0; i < Size2; i++)
			{
				for (int j = 0; j < Size1; j++)//check if a candidate pixel should be deleted
				{
					...
					else
					{
						...
						ind = St2I(a);
						//cout << ind << endl;
						if (ind == 32 || ind == 128 || ind == 1 || ind == 4 || ind == 2 || ind == 8 || ind == 16 || ind == 64)
						{
							ImagedataPOut[BytesPerPixel * ((Size1) * (i)+(j)) + s] = 1;
						}
						else if (ind == 72 || ind == 80 || ind == 10 || ind == 18)
						{
							ImagedataPOut[BytesPerPixel * ((Size1) * (i)+(j)) + s] = 1;
						}
...
void P3_c(int BytesPerPixel, char *a1, char *a2, int Size1, int Size2)
{
	...
	Skeleton(BytesPerPixel, Size1, Size2, ImagedataP, Imagedata);
